...
int Model_OBJ::LoadFile(char *filename) {
...
int Model_OBJ::Load(vector<glm::dvec3> const &vs, vector<glm::ivec3> const &fs) {
  vertices = vs;
  face_indices = fs;
  return 0;
}
...
glm::dvec3 Model_OBJ::Find_Closest(int i) {
  ...
  glm::dvec3 tris[3];
  glm::dvec3 normal;
  for (set<int>::iterator it = v_faces[i].begin(); it != v_faces[i].end(); ++it) {
    int face_ind = *it;
    for (int j = 0; j < 3; ++j)
...
void Model_OBJ::Project_Manifold() {
  ...
  for (int iter = 0; iter < ITER_NUM; ++iter) {
    ...
    for (int i = 0; i < (int)face_indices.size(); ++i) {
      ...
      id[1] = face_indices[i][1];
      id[2] = face_indices[i][2];
      face_normals[i] = glm::normalize(
          glm::cross(vertices[id[1]] - vertices[id[0]], vertices[id[2]] - vertices[id[0]]));
    }
      ...
      if (flag) {
        ...
        for (int j = 0; j < (int)vertex_faces[i].size(); ++j) {
          ...
          while (face_index[t] != i)
            t += 1;
          glm::dvec3 dir = glm::normalize(vertices[face_index[(t + 2) % 3]] -
                                          vertices[face_index[(t + 1) % 3]]);
          glm::dvec3 h = vertices[face_index[(t + 1) % 3]] +
                         glm::dot(vertices[i] - vertices[face_index[(t + 1) % 3]], dir) * dir -
                         vertices[i];
        ...
        for (int j = 0; j < (int)vertex_faces[i].size(); ++j) {
          ...
          face_normals[face] = glm::normalize(
              glm::cross(vertices[face_indices[face][1]] - vertices[face_indices[face][0]],
    ...
    for (int i = 0; i < (int)invalid_vertices.size(); ++i) {
      if (invalid_colors[i] == -1) {
        ...
        for (vector<int>::reverse_iterator it = queue.rbegin(); it != queue.rend(); ++it) {
          ...
          for (int j = 0; j < (int)vertex_faces[id].size(); ++j) {
            ...
            glm::dvec3 dir = glm::normalize(vertices[face_index[(t + 2) % 3]] -
                                            vertices[face_index[(t + 1) % 3]]);
            glm::dvec3 h = vertices[face_index[(t + 1) % 3]] +
                           glm::dot(vertices[id] - vertices[face_index[(t + 1) % 3]], dir) * dir -
                           vertices[id];
            double h_len = glm::length(h);
            h /= h_len;
...
void Model_OBJ::Build_BVH() {
  ...
  for (int i = 0; i < (int)face_indices.size(); ++i) {
    bvs[i] = new BV(vertices[face_indices[i][0]], vertices[face_indices[i][1]],
...
bool Model_OBJ::Split_Grid(map<Grid_Index, int> &vcolor, vector<glm::dvec3> &nvertices,
                           vector<glm::ivec4> &nface_indices, vector<set<int>> &v_faces,
                           vector<glm::ivec3> &triangles) {
  double unit_len = 0;
  v_info.resize(vcolor.size());
  for (map<Grid_Index, int>::iterator it = vcolor.begin(); it != vcolor.end(); ++it) {
  ...
  triangles.clear();
  double half_len =
      glm::length(nvertices[nface_indices[0][1]] - nvertices[nface_indices[0][0]]) * 0.5;
  for (int i = 0; i < (int)nface_indices.size(); ++i) {
    int t = 0;
    ...
    if (t == 4) {
      triangles.push_back(
          glm::ivec3(nface_indices[i][0], nface_indices[i][2], nface_indices[i][1]));
      triangles.push_back(
          glm::ivec3(nface_indices[i][0], nface_indices[i][3], nface_indices[i][2]));
      continue;
    }
    int ind[4];
    for (int j = 0; j < 4; ++j)
  ...
  for (set<int>::iterator it = marked_v.begin(); it != marked_v.end(); ++it) {
      ...
      for (int dimy = -1; dimy < 2; dimy += 2) {
        for (int dimz = -1; dimz < 2; dimz += 2) {
          ...
          if (tree->Is_Exterior(p1)) {
            ...
            int id3 = vcolor[ind3];
            glm::dvec3 norm =
                glm::cross(nvertices[id2] - nvertices[id1], nvertices[id3] - nvertices[id1]);
  ...
  for (set<int>::iterator it = evens.begin(); it != evens.end(); ++it) {
    int i = *it;
    glm::dvec3 dir;
    int count = 0;
    for (int j = 0; j < 8; ++j) {
    ...
    for (set<pair<int, int>>::iterator it1 = p.begin(); it1 != p.end(); ++it1) {
      assert(triangles[it1->first][it1->second] == i);
      if (glm::dot(nvertices[triangles[it1->first][(it1->second + 1) % 3]] - nvertices[i], dir) <
          0) {
  ...
  for (set<int>::iterator it = odds.begin(); it != odds.end(); ++it) {
    int i = *it;
    int k = 0;
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      if (it == edges.end()) {
        ...
        if (it->second.size() == 2) {
          ...
          if (glm::dot(norm1, norm2) > 1 - 1e-7 && glm::length(p1) > 1e-4 &&
              glm::length(p2) > 1e-4) {
            flag = 0;
          }
        }
...
void Model_OBJ::SaveOBJ(const char *filename) {
  ...
  for (int i = 0; i < (int)face_indices.size(); ++i) {
    os << "f " << face_indices[i][0] + 1 << " " << face_indices[i][1] + 1 << " "
       << face_indices[i][2] + 1 << "\n";
  }
  os.close();
}
  ...
  for (int i = 0; i < (int)vertices.size(); ++i) {
    if (color)
      os << vertices[i][0] << " " << vertices[i][1] << " " << vertices[i][2] << " "
         << (int)(colors[i][2] * 255) << " " << (int)(colors[i][1] * 255) << " "
         << (int)(colors[i][0] * 255) << " 255\n";
    else
      os << vertices[i][0] << " " << vertices[i][1] << " " << vertices[i][2] << "\n";
  }
